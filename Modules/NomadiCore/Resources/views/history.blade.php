    ...
    <div class='row'>
        <div class='col-md-12'>
            <ul class="nav nav-tabs">
              ...
              <li role="presentation" @if($mode === 'cafes') class="active" @endif><a href="/user/{{$user->id}}?tab=cafes">新增 ({{$user->entities->count()}})</a></li>
              ...
              <li role="presentation" @if($mode === 'tags') class="active" @endif><a href="/user/{{$user->id}}?tab=tags">標籤 ({{$user->entityTags->count()}})</a></li>
            </ul>
        </div>
    </div>
    <br>
...
<div class='container single-block'>
    <div class='row'>
        <div class='col-md-12 grid'>
            @if ($mode === 'cafes')
                <a class='title' href='/user/{{$user->id}}?tab=cafes'>推薦新增 {{ $user->entities->count() }} 間店家</a>
                <br>
                @foreach($user->entities->sortByDesc('created_at') as $cafe)
            ...
            @elseif ($mode === 'visits')
                ...
                @endforeach
            @elseif ($mode === 'tags')
                <a class='title' href='/user/{{$user->id}}?tab=tags'>加上 {{ $user->entityTags->count() }} 個標籤</a>
                <br>
                @foreach($user->entityTags->sortByDesc('created_at') as $cafeTag)
...
<div class='container summary-block'>
    <div class='row'>
        <div class='col-md-6 grid'>
            <a class='title' href='/user/{{$user->id}}?tab=cafes'>推薦新增 {{ $user->entities->count() }} 間店家</a>
            <br>
            @foreach($user->entities->sortByDesc('created_at')->take(5) as $cafe)
                @include('nomadicore::user/summary/cafe-entry')
            @endforeach
    ...
    <div class='row'>
        ...
        <div class='col-md-6 grid'>
            <a class='title' href='/user/{{$user->id}}?tab=tags'>加上 {{ $user->entityTags->count() }} 個標籤</a>
            <br>
            @foreach($user->entityTags->sortByDesc('created_at')->take(5) as $cafeTag)
                @include('nomadicore::user/summary/tag-entry')
            @endforeach
